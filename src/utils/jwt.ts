...
export const signAccessToken = async (userId: number) => {
    return await signToken(
        { userId, tokenType: 'access' },
        env.JWT_ACCESS_SECRET,
        { expiresIn: env.JWT_ACCESS_EXPIRE_TIME as any }
    );
};

export const signRefreshToken = async (userId: number) => {
    return await signToken({ userId, tokenType: 'refresh' }, env.JWT_REFRESH_SECRET, { expiresIn: env.JWT_REFRESH_EXPIRE_TIME as any });
};
